  ...
  if (volumeSlider) {
    volumeSlider.addEventListener('input', function () {
      ...
      if (playerReady) {
        ...
        // Dragging to 0 should behave like hitting mute (setVolume(0)
        // alone doesn't reliably silence playback), and dragging back up
        // from 0 is the universal "I want sound again" gesture — unmute
        // so the new level is actually heard.
        if (value === 0 && !player.isMuted()) {
          player.mute();
          savedMuted = true;
          try { localStorage.setItem('imperius-player-muted', '1'); } catch (e) {}
          setMuteIcon(true);
